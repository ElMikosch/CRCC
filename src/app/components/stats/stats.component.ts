...
export class StatsComponent implements OnInit {
  ...
  calculatePoints(stats: Stat[]) {
    this.spendPoints = stats.filter(x => !x.isDerived).map(x => x.value).reduce((a, b) => a + b, 0);
  ...
  calculateDerivedStats(stats: Stat[]) {
    ...
    if (currentHP && currentBODY && currentWILL) {
      currentHP.value = currentHP.derivedCalculation(currentBODY.value, currentWILL.value);
    }
